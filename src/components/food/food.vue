...
const ALL = 2;
 ...
 export default {
    ...
    //如果子元素或者父元素 DOM 结构发生改变的时候，必须重新调用 scroll.refresh() 方法重新计算来确保滚动效果的正常。
        ...
        addFirst() {//默认参数为event
            if(!event._constructed){//防止双击
                return;
            }
            this.$emit('cart-add',event.target);//第二个是要传递的参数
    ...
    filters: {
        formatDate(time) {
            ...
            return formatDate(date,'yyyy-MM-dd hh:mm');//date.js
        }
    },
    components: {
        cartcontrol,
...
@import '../../common/stylus/mixin.styl';
    .food
        ...
        transform translate3d(0,0,0) //最终状态
        &.move-enter,&.move-leave-to //动画过度的css样式，enter：进入过渡的开始状态，leave-to:离开过渡的结束状态
            transform translate3d(100%,0,0)
        .image-header
            position relative
            width 100%
            height 0 //用法新颖，宽高相等
            padding-top 100% //相对于宽
            img
                position absolute
                top 0
                left 0
        ...
        .content
            ...
            .price
                ...
                .now
                    ...
                    color rgb(240,20,20)
                .old
                    text-decoration line-through //画线
                    font-size 10px
                    color rgb(147,153,159)
            .cartcontrol-wrapper
                position absolute
